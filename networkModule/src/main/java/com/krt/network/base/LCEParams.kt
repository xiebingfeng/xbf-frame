...
class LCEParams(
        val notStartLce: Boolean = false,    //不启动lce，纯粹的请求网络，用户看不见
        ...
        val notShowContentWhenSuccess: Boolean = false,  //是否  显示内容，（当显示内容时会取消等待状态，主要适用于，网络请求后，立马再请求其它数据）
        val errorWarn: String? = null   //数据错误时的默认提示，替代服务器返回的错误信息
)